...
def plot_square_quartiles(x_values, means, participants, better, ax, percentile=50):
    ...
    # create a dictionary with participants and their corresponding quartile
    participants_quartiles = {}
    if better == "bottom-right":
        ...
        for i, val in enumerate(participants, 0):
            if x_values[i] >= x_percentile and means[i] <= y_percentile:
                participants_quartiles[participants[i]] = 1
            elif x_values[i] >= x_percentile and means[i] > y_percentile:
                participants_quartiles[participants[i]] = 3
            elif x_values[i] < x_percentile and means[i] > y_percentile:
                participants_quartiles[participants[i]] = 4
            elif x_values[i] < x_percentile and means[i] <= y_percentile:
                participants_quartiles[participants[i]] = 2
...
# funtion that splits the analysed participants into four quartiles, according to the asigned score
...
def plot_diagonal_quartiles(x_values, means, participants, better):
    ...
    max_x = max(x_values)
    max_y = max(means)
    # compute the scores for each of the participant. based on their distance to the x and y axis
    scores = []
    ...
    scores_and_values = sorted([[scores[i], x_values[i], means[i], participants[i]] for i, val in enumerate(scores, 0)],
                               reverse=True)
    ...
    # split in quartiles
    participants_quartiles = get_quartile_points(scores_and_values, first_quartile, second_quartile, third_quartile)

    # Plot annotation boxes with info about scores and participant names
    for counter, (scr, x_val, mean_val, participant) in enumerate(scores_and_values):

        # Adjust the y-coordinate of the annotation to avoid overlapping with the diagonal line
        offset  = -80 
        offset2 =   0.5 * max_y 
        ...
        plt.annotate(
            ...
            xy=(x_val, mean_val), xytext=(offset, offset2),
            textcoords='offset points', ha='right', va='bottom',
            bbox=dict(boxstyle='round,pad=0.5', fc='yellow', alpha=0.15),
            size=10,  # Adjust the size of the text
...
# function that prints a table with the list of participants and the corresponding quartiles
def print_quartiles_table(participants_quartiles):
    row_names = participants_quartiles.keys()
    quartiles_1 = participants_quartiles.values()

    colnames = ["Participant", "Quartile"] 
...
def print_chart(challenge_dir, summary_dir, challenge_type, classification_type):
    ...
    for participant_data in aggregation_file["datalink"]["inline_data"]["challenge_participants"]:
        participants.append(participant_data['participant_id'])
    ...
    for i, val in enumerate(participants, 0):
        ...
        ax.errorbar(x_values[i], y_values[i], linestyle='None', marker=markers[i],
                    ...
                    ecolor=colors[i], label=participants[i])

    plt.title(challenge_type, fontsize=18, fontweight='bold', pad=20)  # Ajusta el tamaño y la separación del título
    ax.set_xlabel(x_metric, fontsize=14, labelpad=20)  # Ajusta el tamaño de la fuente del eje x y su distancia del título
    ax.set_ylabel(y_metric, fontsize=14, labelpad=20)  # Ajusta el tamaño de la fuente del eje y y su distancia del título

    # plt.title(challenge_type, fontsize=18, fontweight='bold')
    # ax.set_xlabel(x_metric, fontsize=14)  # Ajusta el tamaño de la fuente del eje x
    # ax.set_ylabel(y_metric, fontsize=14)  # Ajusta el tamaño de la fuente del eje y

    # Put a legend below current axis
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.18), markerscale=0.7,
    ...
    p_frontX, p_frontY = pareto_frontier(x_values, y_values, maxX=True, maxY=True)
    ...
    if classification_type == "SQR":
        participants_quartiles = plot_square_quartiles(x_values, y_values, participants, better, ax)
        print_quartiles_table(participants_quartiles)

    elif classification_type == "DIAG":
        participants_quartiles = plot_diagonal_quartiles(x_values, y_values, participants, better)
        print_quartiles_table(participants_quartiles)
    ...
    outname = os.path.join(challenge_dir, out_id.replace(" ", "_") + "_benchmark_" + classification_type.replace(" ", "_") + ".svg")
...
def print_barplot(challenge_dir, aggregation, challenge_acronym):
    ...
    metric_name = aggregation["datalink"]["inline_data"]["visualization"]["metric"]

    fig, ax = plt.subplots(figsize=(12, 8))  # Ajusta el tamaño de la figura

    ax.bar(participants, values, color=orange_hex, edgecolor=orange_hex)

    ax.set_xlabel("Participants", fontsize=14, labelpad=20)  # Ajusta el tamaño de la fuente del eje x y su distancia del título
    ax.set_ylabel(f"{metric_name.capitalize()}", fontsize=14, labelpad=20)  # Ajusta el tamaño de la fuente del eje y y su distancia del título
    ax.set_title(f"{metric_name.capitalize()} Bar Plot in Challenge {challenge_acronym}", fontsize=16, fontweight='bold', pad=20)  # Ajusta el tamaño de la fuente del título y su distancia del gráfico
    ax.legend(fontsize=12)  # Ajusta el tamaño de la fuente de la leyenda
    ax.tick_params(axis='y', which='major',labelsize=12)  # Ajusta la separación entre el título del eje y las etiquetas del eje y, y el tamaño de las etiquetas del eje y
    ax.tick_params(axis='x', which='major', labelsize=14)  # Ajusta la separación entre el título del eje y las etiquetas del eje x, y el tamaño de las etiquetas del eje x
    ...
    outname = os.path.join(challenge_dir,
                           out_id.replace(" ", "_") + "_benchmark_" + metric_name.replace(" ", "_") + "_barplot.svg")
    fig.savefig(outname, dpi=100)
